...
app/(root)/story/[id]/ → single story detail page ✅ implemented
...
app/api/stories/ → GET feed (paginated), POST create story ✅ implemented
app/api/stories/[id]/ → GET one, PATCH update, DELETE ✅ implemented
app/api/stories/[id]/vote/ → POST upvote or downvote (body: { type: "up" | "down" }) ✅ implemented
app/api/stories/[id]/comments/ → GET comments, POST add comment ✅ implemented
app/api/stories/[id]/comments/[cid]/ → DELETE comment ✅ implemented
...
Never return raw Mongoose documents. Always use .lean() and strip: passwordHash, \_\_v.
Return shapes:
...
DTO types never include passwordHash or \_\_v.
...
user-profile: done
family-tree-view: done
add-family-member: done
add-relationship: done
connection-request: done
story-create: done
story-feed: done
story-vote: done
story-comment: done
